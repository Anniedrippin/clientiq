...
async function request(method, path, { body, event } = {}) {
  ...
  const eventName =
    event || `${method.toLowerCase()}_${path.replace(/\W+/g, "_")}`;
...
export const api = {
  ...
  getKpis: (region) =>
    request(
      "GET",
      `/api/kpi${region ? `?region=${encodeURIComponent(region)}` : ""}`,
      { event: "kpi_fetch" },
    ),

  ask: (question, region) =>
    request("POST", "/api/analysis/ask", {
      body: { question, region },
  ...
  history: () =>
    request("GET", "/api/analysis/history", {
      event: "analysis_history_fetch",
    }),

  exportPdf: (requestId) =>
    request("GET", `/api/export/${requestId}/pdf`, { event: "pdf_export" }),
};
